<template>
  <article aria-label="Web development">
    <h2>Web development</h2>
    <section
      id="sigic"
      aria-labelledby="sigic-label"
    >
      <a
        href="https://idce.dev.geoint.mx/admin/"
        target="_blank"
        rel="noopener noreferrer"
      >
        <h3 id="sigic-label">Sistema Integral de Gestión de Información Científica (SIGIC)</h3>
      </a>
    </section>

    <section
      id="gema"
      aria-labelledby="gema-label"
    >
      <a
        href="https://gema.conahcyt.mx/"
        target="_blank"
        rel="noopener noreferrer"
      >
        <h3 id="gema-label">Generador y visualizador de Mapas Interactivo (GEMA)</h3>
      </a>
    </section>

    <section
      id="sisdai"
      aria-labelledby="sisdai-label"
    >
      <a
        href="https://sisdai.org/"
        target="_blank"
        rel="noopener noreferrer"
      >
        <h3 id="sisdai-label">Sistema de diseño y Accesibilidad para la Investigación (SISDAI)</h3>
      </a>

      <div style="display: flex; gap: 24px">
        <ul>
          <li>
            <a
              href="http://sisdai-css.conahcyt.mx"
              target="_blank"
              rel="noopener noreferrer"
            >sisdai-css</a>
          </li>
          <li>
            <a
              href="http://sisdai-graficas.conahcyt.mx"
              target="_blank"
              rel="noopener noreferrer"
            >sisdai-graficas</a>
          </li>
          <li>
            <a
              href="http://sisdai-mapas.conahcyt.mx"
              target="_blank"
              rel="noopener noreferrer"
            >sisdai-mapas</a>
          </li>
        </ul>
        <ul>
          <li>
            <a
              href="http://sisdai-componentes.conahcyt.mx"
              target="_blank"
              rel="noopener noreferrer"
            >sisdai-componentes</a>
          </li>
          <li>
            <a
              href="https://www.npmjs.com/org/centrogeomx"
              target="_blank"
              rel="noopener noreferrer"
            >npmjs</a>
          </li>
        </ul>
      </div>
    </section>

    <section
      id="eni"
      aria-labelledby="eni-label"
    >
      <a
        href="https://eni.conacyt.mx/"
        target="_blank"
        rel="noopener noreferrer"
      >
        <h3 id="eni-label">Ecosistemas Nacionales Informáticos (ENI)</h3></a>
      <details>
        <summary
          id="button-label"
          aria-labelledby="button-label eni-label"
        >
          Mostrar más
        </summary>

        <ul>
          <li>
            <a
              href="https://salud.conacyt.mx/"
              target="_blank"
              rel="noopener noreferrer"
            >
              Salud
            </a>
            <ul>
              <li>
                <a
                  href="https://salud.conacyt.mx/vih/"
                  class="color-white"
                  target="_blank"
                  rel="noopener noreferrer"
                >Red de transmisión y vigilancia de farmacorresistencia de
                  <strong>VIH</strong> en la CDMX</a>
              </li>
              <li>
                <a
                  href="https://salud.conacyt.mx/coronavirus/"
                  class="color-white"
                  target="_blank"
                  rel="noopener noreferrer"
                >Conhacyt frente a la <strong>Covid-19</strong></a>
                <ul>
                  <li>
                    <a
                      href="https://salud.conacyt.mx/coronavirus/variantes/"
                      target="_blank"
                      rel="noopener noreferrer"
                    >Vigilancia de <strong>variantes</strong> del virus
                      SARS-CoV-2</a>
                  </li>
                  <li>
                    <a
                      href="https://salud.conacyt.mx/coronavirus/vacunacion/"
                      target="_blank"
                      rel="noopener noreferrer"
                    >Plataforma para la Planeación Logística de la Camapa
                      Nacaional de <strong>Vacunación</strong> Covid-19</a>
                  </li>
                  <li>
                    <a
                      href="https://salud.conahcyt.mx/coronavirus/investigacion/"
                      target="_blank"
                      rel="noopener noreferrer"
                    >Productos de <strong>investigación</strong> y modelado de
                      datos Covid-19</a>
                  </li>
                </ul>
              </li>
            </ul>
          </li>

          <li>
            <a
              href="https://alimentacion.conacyt.mx/"
              target="_blank"
              rel="noopener noreferrer"
            >
              Soberanía alimentaria</a>
            <ul>
              <li>
                <a
                  href="https://alimentacion.conacyt.mx/glifosato/"
                  target="_blank"
                  rel="noopener noreferrer"
                >Búsqueda de alternativas al <strong>glifosato</strong></a>
              </li>
              <li>
                <a
                  href="https://alimentacion.conacyt.mx/piesagiles/"
                  target="_blank"
                  rel="noopener noreferrer"
                >Pies ágiles</a>
              </li>
            </ul>
          </li>

          <li>
            <a
              href="http://socioecologia.conahcyt.mx"
              target="_blank"
              rel="noopener noreferrer"
            >Sistemas socioecológicos</a>
            <ul>
              <li>
                <a
                  href="https://socioecologia.conacyt.mx/texcoco/"
                  target="_blank"
                  rel="noopener noreferrer"
                >Rescate del Lado de <strong>Texcoco</strong></a>
              </li>
            </ul>
          </li>

          <li>
            <a
              href="http://cultura.conahcyt.mx"
              target="_blank"
              rel="noopener noreferrer"
            >Cultura</a>
            <ul>
              <li>
                <a
                  href="http://cultura.conahcyt.mx/pueblosindigenas"
                  target="_blank"
                  rel="noopener noreferrer"
                >Pueblos y <strong>Comunidades Indígenas</strong> y
                  Afromexicanas</a>
              </li>
            </ul>
          </li>

          <li>
            <a
              href="https://energia.conacyt.mx/"
              target="_blank"
              rel="noopener noreferrer"
            >Energía y cambio climático</a>
            <ul>
              <li>
                <a
                  href="https://energia.conacyt.mx/sargazo/"
                  target="_blank"
                  rel="noopener noreferrer"
                >Arribo del <strong>sargazo</strong> pelágico a las costas
                  mexicanas</a>
              </li>
              <li>
                <a
                  href="https://energia.conacyt.mx/planeas/"
                  target="_blank"
                  rel="noopener noreferrer"
                >Planeas</a>
              </li>
            </ul>
          </li>

          <li>
            <a
              href="http://seguridad.conahcyt.mx"
              target="_blank"
              rel="noopener noreferrer"
            >Seguridad humana</a>
            <ul>
              <li>
                <a
                  href="http://seguridad.conahcyt.mx/guerra-sucia"
                  target="_blank"
                  rel="noopener noreferrer"
                >Desenredando la Guerra sucia</a>
              </li>
            </ul>
          </li>

          <li>
            <a
              href="https://agua.conahcyt.mx/"
              target="_blank"
              rel="noopener noreferrer"
            >Agua</a>
            <ul>
              <li>
                <a
                  href="https://agua.conacyt.mx/cuencas/"
                  target="_blank"
                  rel="noopener noreferrer"
                >Aguas y <strong>cuencas</strong> en México</a>
              </li>
              <li>
                <a
                  href="https://agua.conacyt.mx/atoyac/"
                  target="_blank"
                  rel="noopener noreferrer"
                >Reapropiación comunitaria de la cuenca Atoyac-Zahuapan</a>
              </li>
              <li>
                <a
                  href="https://agua.conacyt.mx/trasvases/"
                  target="_blank"
                  rel="noopener noreferrer"
                >Trasvases y Justicia Hídrica</a>
              </li>
            </ul>
          </li>
          <li>
            <a
              href="https://toxicologia.conacyt.mx/"
              target="_blank"
              rel="noopener noreferrer"
            >Agentes tóxicos y procesos contaminantes</a>
            <ul>
              <li>
                <a
                  href="https://toxicologia.conahcyt.mx/resa-atoyac/"
                  target="_blank"
                  rel="noopener noreferrer"
                >Región de Emergencia Sanitaria y Ambiental:
                  <strong>Cuenca del alto Atoyac</strong></a>
              </li>
            </ul>
          </li>

          <li>
            <a
              href="http://vivienda.conahcyt.mx"
              target="_blank"
              rel="noopener noreferrer"
            >Vivienda</a>
          </li>

          <li>
            <a
              href="http://educacion.conahcyt.mx"
              target="_blank"
              rel="noopener noreferrer"
            >Educación</a>
          </li>
        </ul>
      </details>
    </section>

    <br>
    <nuxt-link to="/">[Go back]</nuxt-link>
  </article>
</template>
