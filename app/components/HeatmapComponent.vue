...
// console.log('vvariables', vvariables)
...
function calculoHeatmap(tipoVariante) {
  ...
  if (tipoVariante.value === 'VTODAS') {
    ...
    for (let i = 0; i < keyEstado.length; i++) {
      ...
      const filtroEstado = Object.values(datosJSON).filter(
      ...
      for (let j = 0; j < keyTipoVariante.length; j++) {
        ...
        filtroTipoVariante.forEach((element) => {
          ...
          valor = Number.parseFloat(element.cantidad_muestras)
        })
...
function configurandoDimensionesParaHeatmap() {
  if (window.innerWidth >= width_limit) {
    ...
    x.value = d3
      .scaleBand()
      .range([50, width.value])
      .domain(keyAbreviatura.value)
      .padding(0.03)
    y.value = d3
      .scaleBand()
      .range([height.value, 0])
      .domain(keyVariante.value)
      .padding(0.03)
...
function contenidoTooltip(datum) {
  return `${
    varianteSeleccionada.value === 'VTODAS'
...
function actualizandoHeatmap() {
  heatmap.value
    ...
    .attr('x', d =>
      window.innerWidth >= width_limit
        ? x.value(d.abreviatura)
        : x.value(d.variable),
    )
    .attr('y', d =>
      window.innerWidth >= width_limit
...
onMounted(() => {
  ...
  svg.value = d3.select(`div#${props.mapaCalorId} svg.svg-heatmap`)
  ...
  eje_x_bottom.value = grupo_contenedor_ejes
    .append('g')
...
watch(
  ...
  () => {
    ...
    configurandoDimensionesParaSVG()
    // console.log('lista_filtros_activos')
    calculoHeatmap(varianteSeleccionada)
    configurandoDimensionesParaHeatmap()
    if (window.innerWidth >= width_limit) {
      creandoHeatmapDesktop()
  ...
  <div class="mapa-calor">
    <div class="encabezado">
      <h4 class="titulo-visualizacion">
        ...
        {{
          variantes
            .find(variante => variante.clave === varianteSeleccionada)
            .nombre.toLowerCase()
        }}
      </h4>
      ...
      <div class="checks">
        ...
        >
          <ClientOnly>
            ...
            >
              <span class="categoria-texto">{{ variable.nombre }} {{ lista_filtros_activos[i] }}</span>
            </CheckboxColor>
          </ClientOnly>
        </div>
      </div>
    ...
    >
      <div :id="mapaCalorId">
        <svg
          ...
          class="svg-heatmap"
        >
          <g class="grupo-contenedor-de-heatmap" />
          <g class="grupo-contenedor-de-ejes" />
        </svg>
      </div>
