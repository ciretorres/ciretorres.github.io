<template>
  <div class="contenedor-area">
    ...
    <div class="dai-contenedor-area" v-bind:id="area_id">
...
export default {
  ...
  mounted() {
    ...
    this.texto_x = this.grupo_contenedor_ejes
      .append('text')
      .text(this.titulo_eje_x)
      .attr('class', 'label-x')
      .style('font-weight', '600')
    this.texto_y = this.grupo_contenedor_ejes
      .append('text')
    ...
    this.eje_x = this.grupo_contenedor_ejes.append('g').attr('class', 'eje-x')
  ...
  methods: {
    configurandoDimensionesParaSVG() {
      ...
      this.grupo_contenedor_ejes.attr(
        'transform',
      ...
      // Labels title
      this.texto_x
        .attr('transform', `translate(${this.width * 0.5}, ${this.height + 25})`)
        .style('text-anchor', 'middle')
        .style('font-size', '10px')
        .style('dominant-baseline', 'hanging')
        .style('color', '#000')
      this.texto_y
        .attr('transform', `translate(${-this.margin.left}, ${this.height * 0.5}) rotate(-90)`)
        .style('text-anchor', 'middle')
        .style('font-size', '10px')
        .style('dominant-baseline', 'hanging')
        .style('color', '#000')
    ...
    configurandoDimensionesParaArea() {
      ...
      // Color lines
      this.xAxisG.selectAll('.tick line').style('stroke', '#000')

      // Add Y axis
      this.yAxis = d3.axisLeft(this.yScale).tickSize(-this.width)
      // Call Y axis
      this.yAxisG = this.eje_y.append('g').call(this.yAxis)
      // Remove domain line
      ...
      this.yAxisG
        ...
        .style('color', '#000')
      // Color lines
      this.yAxisG.selectAll('.tick line').style('stroke', '#000')
    ...
    creandoArea() {
      ...
      this.grupo_contenedor.selectAll('path.paths-area').remove()
    ...
    mostrarTooltip(evento, datum) {
      ...
      //   .text("Fecha:");
      d3.select(this.$refs.tooltip_cifra).html(`Variable: ${
        datum.date.getDate() + '-' + (+datum.date.getMonth() + 1) + '-' + datum.date.getFullYear()
      } 
    ...
    cerrarTooltip() {
       ...
       */
      this.tooltip.style('visibility', 'hidden').style('left', '0').style('top', '0')
    }
  },
  computed: {}
}
