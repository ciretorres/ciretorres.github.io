...
export default {
  ...
  watch: {
    lista_filtros_activos(nv) {
      ...
      this.categorias_checkeadas = this.variables_graf.map((d, i) => this.lista_filtros_activos[i] ? d.id : '').filter((d) => d !== '');
      ...
      */
      if (tipoVariante === 'VTODAS') {
        ...
        for (let i = 0; i < keyEstado.length; i++) {
          ...
          const filtroEstado = Object.values(consorcioHeatmap)
        ...
        const filtroTipoVariante = Object.values(consorcioHeatmap)
