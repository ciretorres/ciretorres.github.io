  ...
  >
    ...
    <div class="contenedor-vis" id="contenedor_vis">
      ...
      <div id="contenedor_svg">
        <svg class="svg-heatmap" ref="heatmap">
      ...
      <div class="tooltip-mapa-calor" ref="tooltip">
        <div class="contenedor-boton-cerrar">
          ...
          <button class="boton-cerrar-tooltip" @click="cerrarTooltip()">
            <img src="@/assets/imgs/cerrar.svg" alt="" class="img-cerrar">
          </button>
        </div>
...
export default {
  ...
  methods: {
      ...
      */
      if (window.innerWidth >= this.width_limit) { // 769
        ...
        if (this.varianteSeleccionada === 'VTODAS'){
          ...
          this.eje_y.select('.domain')
            .remove();
          this.eje_y_abr.style('opacity', '0');
        }
      } else {
        // Mobile
        // Build X, Y scales:
    ...
    positionTooltip(e) {
      return {
        x: (60+parseInt(e.target.attributes['x'].value)+this.$refs.tooltip.clientWidth)
          > this.$refs.heatmap.clientWidth
            ? e.pageX - this.$refs.tooltip.clientWidth - 5
            : e.pageX + 15,
        y: (10+parseInt(e.target.attributes['y'].value)+parseInt(e.target.attributes['height'].value)
          + this.$refs.tooltip.clientHeight)> this.$refs.heatmap.clientHeight
            ? e.pageY - this.$refs.tooltip.clientHeight
            : e.pageY + 15,
      }
    },
    mostrarTooltip(evento, datum) {
       ...
       */
      const position = this.positionTooltip(evento)
      ...
      d3.select(this.$refs.tooltip_cifra)
        .html(this.contenidoTooltip(datum));
      // d3.select(this.$refs.tooltip_cifra)
